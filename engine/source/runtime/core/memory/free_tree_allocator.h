...
{
    template<typename T>
    class FreeTreeAllocator : public Allocator<T>
    {
    private:
        struct Header
        {
    ...
    public:
        ...
        template<typename U>
        FreeTreeAllocator(const FreeTreeAllocator<U>& other) : Allocator<T>(other.m_size)
        {
            FreeTreeAllocator(other.m_size);
            (void)other;
        }
    ...
    template<typename T>
    FreeTreeAllocator<T>::FreeTreeAllocator() : Allocator<T>(1024)
    {
        FreeTreeAllocator(1024);
    }

    template<typename T>
    FreeTreeAllocator<T>::FreeTreeAllocator(const std::size_t size) : Allocator<T>(size)
    {
    ...
    template<typename T>
    FreeTreeAllocator<T>::~FreeTreeAllocator()
    {
    ...
    template<typename T>
    T* FreeTreeAllocator<T>::allocate(const std::size_t size_, const std::size_t alignment)
    {
    ...
    template<typename T>
    void FreeTreeAllocator<T>::deallocate(T* ptr, std::size_t n)
    {
    ...
    template<typename T>
    void FreeTreeAllocator<T>::reset()
    {
        init();
    }

    template<typename T>
    void FreeTreeAllocator<T>::init()
    {
    ...
    template<typename T>
    void FreeTreeAllocator<T>::coalescence(RBTree::Node* curr)
    {
    ...
    template<typename T>
    std::size_t FreeTreeAllocator<T>::get_root_node_padding()
    {

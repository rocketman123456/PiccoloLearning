...
namespace Piccolo
{
    ...
    // {
    //     Matrix4x4 directional_light_proj_view = calculate_directional_light_camera(*this, *camera);
    ...
    //     ClusterFrustum frustum = create_cluster_frustum_from_matrix(directional_light_proj_view, -1.0, 1.0, -1.0, 1.0, 0.0, 1.0);
    ...
    //         if (tiled_frustum_intersect_box(frustum, bounding_box_transform(mesh_asset_bounding_box, entity.m_model_matrix)))
    //         {
    ...
    //         for (size_t i = 0; i < point_light_num; i++)
    //         {
    //             if (!box_intersects_with_sphere(bounding_box_transform(mesh_asset_bounding_box, entity.m_model_matrix), point_lights_bounding_spheres[i]))
    //             {
    ...
    //     ClusterFrustum f = create_cluster_frustum_from_matrix(proj_view_matrix, -1.0, 1.0, -1.0, 1.0, 0.0, 1.0);

    //     for (const RenderEntity& entity : m_render_entities)
    //     {
    //         BoundingBox mesh_asset_bounding_box {entity.m_bounding_box.getMinCorner(), entity.m_bounding_box.getMaxCorner()};

    //         if (tiled_frustum_intersect_box(f, bounding_box_transform(mesh_asset_bounding_box, entity.m_model_matrix)))
    //         {

...
namespace Piccolo
{
    ...
    public:
        ...
        // virtual void
        // createBuffer(RHIDeviceSize size, RHIBufferUsageFlags usage, RHIMemoryPropertyFlags properties, RHIBuffer*& buffer, RHIDeviceMemory*& buffer_memory) =
        // 0; virtual void createBufferAndInitialize(RHIBufferUsageFlags    usage,
        ...
        //                                           VmaAllocation*                 pAllocation,
        //                                           VmaAllocationInfo*             pAllocationInfo) = 0;
        // virtual void copyBuffer(RHIBuffer* srcBuffer, RHIBuffer* dstBuffer, RHIDeviceSize srcOffset, RHIDeviceSize dstOffset, RHIDeviceSize size)           =
        // 0;
        ...
        //                                  uint32_t               height)                                                             = 0;
        // virtual void
        // cmdCopyBuffer(RHICommandBuffer* commandBuffer, RHIBuffer* srcBuffer, RHIBuffer* dstBuffer, uint32_t regionCount, RHIBufferCopy* pRegions)         =
        // 0; virtual void cmdDraw(RHICommandBuffer* commandBuffer, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance)
        // = 0; virtual void cmdDispatch(RHICommandBuffer* commandBuffer, uint32_t groupCountX, uint32_t groupCountY, uint32_t groupCountZ) = 0; virtual void
        // cmdDispatchIndirect(RHICommandBuffer* commandBuffer, RHIBuffer* buffer, RHIDeviceSize offset)                                        = 0; virtual
        ...
        // 0; virtual void updateDescriptorSets(uint32_t                     descriptorWriteCount,
        //                                   const RHIWriteDescriptorSet* pDescriptorWrites,
        //                                   uint32_t                     descriptorCopyCount,
        //                                   const RHICopyDescriptorSet*  pDescriptorCopies)                                                                  =
        //                                   0;
        // virtual bool queueSubmit(RHIQueue* queue, uint32_t submitCount, const RHISubmitInfo* pSubmits, RHIFence* fence)                                   =
        // 0; virtual bool queueWaitIdle(RHIQueue* queue) = 0; virtual void resetCommandPool() = 0; virtual void waitForFences() = 0;
